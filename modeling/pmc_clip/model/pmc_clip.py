...
class PMC_CLIP(nn.Module):
    ...
    def encode_text(self, batch, image_features):
        ...
        text_features = x[torch.arange(x.shape[0]), last_token_index[:, 1]] # the 0-index of each row
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD # (bs, 79, 768)
        fusion_features = x # Modify: extract fushion features
        ...
        attentions = None
        text_output = dict.fromkeys(["text_features", "bert_prediction", "attentions", "encoded_label", "fusion_features", "encoded_input"], None)
    ...
    def forward(self, batch):
        ...
        text_features = F.normalize(text_output["text_features"], dim=-1) # (bs, 768)
